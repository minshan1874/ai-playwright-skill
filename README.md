...
| 通过 | 失败 | 不稳定 | 跳过 | 本次未执行 | 未自动化 | 通过率 |
| 3 | 1 | 0 | 0 | 0 | 1 | 75% |

## 二、用例执行明细   ← 按模块分组，逐条列出状态
## 三、失败详情       ← 错误信息 + 失败截图 + trace 回放文件 + 步骤时间线
## 四、未自动化用例（覆盖缺口）   ← 从没写过自动化代码的，单独列出
## 五、本次未执行用例（筛选执行） ← 只在你要求「只跑一部分」时才出现
...
**「没写」和「这次没跑」分得很清楚。** 如果你说「只跑登录相关的 3 条」，其余用例
会列在第五节并写明是筛选造成的，**不会**被算成覆盖缺口；反过来，真正的缺口也不会
用「本次未执行」糊弄过去。

**每条失败都有证据。** 失败截图、可以逐步回放的 trace 文件
（能看到每一步的页面快照、网络请求、控制台输出），以及一张**步骤时间线** ——
直接告诉你失败发生在哪一步，异步任务（比如文生图）是卡在排队还是真的失败了。
...
**用 Google / 企业微信 / 钉钉等第三方登录的**，要说明这一点：

```
系统用 Google 账号登录，没有账号密码。
```

第三方登录会被对方拦截自动化浏览器，所以 AI 会让你**自己手工登录一次**把登录态存下来
（扫码、短信验证码、两步验证都可以在这一步完成），之后所有用例复用这份登录态。
两步登录（先输账号、点继续、再输密码）也支持，说清楚流程即可。
...
自动化测试的真实成本，**不在写 Playwright 代码，而在写代码之前和之后**：

| 环节 | 手工做法 | 这个 skill 替你做的 |
| --- | --- | --- |
| 读用例文档 | 人工一条条看 | 解析 Excel/CSV，转成结构化用例 |
| 确认测什么 | 口头或文档对齐 | 出测试计划，**你确认后**才执行 |
| 找元素定位 | F12 一个个试 | 打开页面抓语义树，给出按稳定性排序的定位器 |
| 写测试代码 | 一条条手写 | 生成可复现的 `.spec.ts` |
| 对回用例编号 | 手工比对 | 结果自动关联 TC-ID，可追溯到 Excel 行号 |
| 出报告 | 手工整理 | Markdown 报告 + HTML + JSON，含失败截图与 trace |

一句话：**把「已经写好的用例表」变成「可重复执行的自动化测试」。**
省掉中间那层重复劳动，不替你决定测什么。

最适合的场景：**核心链路的回归测试** —— 每次发版前都要重跑的那几十条用例。

## 不做什么

单元测试、接口/API 测试、性能压测、视觉回归、CI 平台对接、测试管理平台同步。

**这不是缺口，是刻意划的边界。** 用端到端测试去凑这些，得到的只会是又慢又脆的东西：
接口该用接口测试，单元该用单元测试，性能该用压测工具。
端到端测试只该覆盖「用户真实走一遍」的那部分 —— 而它恰好最贵、最值得被自动化。

## 和「让 AI 直接写 Playwright 脚本」有什么不同

| | 直接让 AI 写脚本 | 这个 skill |
| --- | --- | --- |
| 动手前 | 直接开始写 | **先出测试计划，等你确认** |
| 测了什么 | 不清楚 | 逐条列出，并标注能否自动化 |
| 没测什么 | 通常不说 | **单列一节**，且**不计入通过率** |
| 出问题怎么查 | 只有一段报错 | 失败截图 + trace，可逐步回放 DOM 与网络 |
| 环境故障 | 容易当成「测试失败」 | 区分环境故障与产品缺陷，不混为一谈 |

三条不可协商的约束写在 [`SKILL.md`](skill/SKILL.md) 里：
计划必须先经确认、绝不伪造结果、不污染被测项目。
